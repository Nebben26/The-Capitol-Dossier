...
import {
  ...
  biggestMovers,
  breakingMarkets,
  homepageWhaleActivity as whaleActivity,
  treemapData,
  HOMEPAGE_CATEGORIES as categories,
} from "@/lib/mockData";
...
function CustomTreemapContent(props: {
  ...
  const bgOpacity = Math.min(Math.abs(change) / 15, 0.7);
  return (
    <g>
      <rect
        x={x}
  ...
  return (
    <div className="flex flex-col items-center">
      <svg viewBox="0 0 200 115" className="w-48 h-auto">
        ...
        <text x="100" y="82" textAnchor="middle" fill="#e2e8f0" fontSize="22" fontWeight="700">
          {value}
        </text>
      </svg>
...
export default function HomePage() {
  ...
  const sortedMovers = useMemo(() => {
    ...
    return [...biggestMovers].sort((a, b) => {
      let aVal: number, bVal: number;
      if (sortKey === "vol") {
        aVal = parseVol(a.vol);
        bVal = parseVol(b.vol);
      } else {
        aVal = Math.abs(a[sortKey]);
        bVal = Math.abs(b[sortKey]);
      }
  ...
  const SortIcon = ({ col }: { col: SortKey }) => {
    ...
    return sortDir === "desc" ? (
      <ChevronDown className="size-3 text-[#57D7BA]" />
    ) : (
      <ChevronUp className="size-3 text-[#57D7BA]" />
    );
  };
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 py-5 space-y-5">
            ...
            {/* ─── HERO: MARKET PULSE ──────────────────────────────── */}
            <Card className="bg-[#222638] border-[#2a2f45] overflow-hidden relative">
                ...
                <div className="flex flex-col lg:flex-row items-center gap-6">
                  ...
                  <div className="flex-1 text-center lg:text-left">
                    <div className="flex items-center justify-center lg:justify-start gap-2 mb-2">
                      ...
                      <span className="text-xs text-[#8892b0]">Updated 2m ago</span>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-[#2a2f45] hover:bg-transparent">
                          ...
                          <TableHead className="text-[10px] text-[#8892b0] font-medium w-16">TREND</TableHead>
                          <TableHead
                            className="text-[10px] text-[#8892b0] font-medium cursor-pointer hover:text-[#57D7BA] transition-colors w-14"
                            onClick={() => handleSort("price")}
                          >
                            <span className="flex items-center gap-0.5">PRICE <SortIcon col="price" /></span>
                          </TableHead>
                          <TableHead
                            className="text-[10px] text-[#8892b0] font-medium cursor-pointer hover:text-[#57D7BA] transition-colors w-16"
                            onClick={() => handleSort("change")}
                          >
                            <span className="flex items-center gap-0.5">24H <SortIcon col="change" /></span>
                          </TableHead>
                          <TableHead
                            className="text-[10px] text-[#8892b0] font-medium cursor-pointer hover:text-[#57D7BA] transition-colors w-16 pr-4"
                      ...
                      <TableBody>
                        {sortedMovers.map((m, i) => (
                          <TableRow
                            key={m.id}
                            className="border-[#2a2f45]/50 hover:bg-[#57D7BA]/5 cursor-pointer transition-colors group/row"
                          ...
                          >
                            <TableCell className="pl-4 py-2.5">
                              <Tooltip>
                                <TooltipTrigger>
                                  <Link
                                    href={`/markets/${m.id}`}
                                    className="text-xs font-medium text-[#e2e8f0] hover:text-[#57D7BA] transition-colors leading-tight line-clamp-2 text-left"
                                  >
                                    {m.q}
                                  </Link>
                                </TooltipTrigger>
                                      ...
                                      <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={m.spark}>
                                          ...
                                          <Area
                                            type="monotone"
                                            dataKey="v"
                                            stroke={m.change >= 0 ? "#22c55e" : "#ef4444"}
                                            fill={`url(#tip-${m.id})`}
                                            strokeWidth={1.5}
                                          />
                                        </AreaChart>
                            ...
                            <TableCell className="py-2.5">
                              <span
                                className={`flex items-center gap-0.5 font-mono text-xs font-semibold ${
                                  m.change >= 0 ? "text-[#22c55e]" : "text-[#ef4444]"
                                }`}
                              >
                                {m.change >= 0 ? (
                            ...
                            <TableCell className="pr-4 py-2.5">
                              <span className="font-mono text-[11px] text-[#8892b0]">${m.vol}</span>
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-[#8892b0] flex items-center gap-1">
                        <span className="size-1.5 rounded-full bg-[#22c55e] animate-pulse-glow" />
                        Live
                      </span>
                    </div>
                  </CardHeader>
                        ...
                        <div className="flex items-start gap-3 p-2.5 rounded-lg hover:bg-[#57D7BA]/5 transition-all border border-transparent hover:border-[#57D7BA]/10">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 mb-1">
                              {m.hot && (
                                <Flame className="size-3 text-[#f59e0b] shrink-0" />
                              )}
                              <span className="text-[10px] text-[#8892b0]">{m.time}</span>
                            </div>
                            <p className="text-xs font-medium text-[#e2e8f0] group-hover:text-[#57D7BA] transition-colors leading-snug">
                              {m.title}
                            </p>
                            <div className="flex items-center gap-3 mt-1.5">
                              <span className="font-mono text-xs font-semibold text-[#57D7BA]">
                                {m.price}¢
                              </span>
                          ...
                          <div className="shrink-0 flex items-center gap-1 px-2 py-1 rounded-md bg-[#1a1e2e] border border-[#2a2f45]">
                            <Eye className="size-3 text-[#8892b0]" />
                            <span className="text-[10px] text-[#8892b0] font-mono">
                              {(parseFloat(m.vol) * 12.3).toFixed(0)}
                            </span>
                          </div>
                        </div>
                      </Link>
              ...
              <div className="lg:col-span-3">
                <Card className="bg-[#222638] border-[#2a2f45]">
                  ...
                  <CardContent className="space-y-2 pb-3">
                    {whaleActivity.map((w, i) => (
                      <div
                        key={w.id}
                        className="p-2.5 rounded-lg border border-[#2a2f45] hover:border-[#57D7BA]/20 transition-all animate-slide-up"
                        style={{ animationDelay: `${i * 100}ms` }}
                      >
                        <div className="flex items-center justify-between mb-1.5">
                          <Link
                            href={`/whales/${w.id}`}
                            className="flex items-center gap-1.5 hover:text-[#57D7BA] transition-colors"
                          >
                            <div className="size-5 rounded-full bg-gradient-to-br from-[#57D7BA] to-[#8b5cf6] flex items-center justify-center text-[8px] font-bold text-[#0f1119]">
                              #{w.rank}
                            </div>
                        ...
                        <Link
                          ...
                          className="text-[11px] text-[#8892b0] hover:text-[#57D7BA] transition-colors line-clamp-1"
                        >
                          {w.market}
                        </Link>
                        <div className="flex items-center gap-1 mt-1.5">
                          <div className="flex-1 h-1 rounded-full bg-[#1a1e2e] overflow-hidden">
                            <div
                              className="h-full rounded-full bg-[#57D7BA]"
                              style={{ width: `${w.acc}%` }}
                            />
                          </div>
                          <span className="text-[9px] text-[#57D7BA] font-mono font-medium">
                            {w.acc}% acc
                          </span>
                        </div>
                      </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <CardDescription className="text-[10px] flex items-center gap-3">
                    <span className="flex items-center gap-1">
                      <span className="size-2 rounded-sm bg-[#22c55e]" /> Positive 24h
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="size-2 rounded-sm bg-[#ef4444]" /> Negative 24h
                    </span>
