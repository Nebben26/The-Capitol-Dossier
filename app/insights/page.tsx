      ...
      <div className="max-w-[1440px] mx-auto px-4 py-5">
        <div className="flex items-center justify-center py-24">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-[#8892b0]">Loading news feed…</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-center justify-center py-24">
          <div className="text-center max-w-md space-y-4">
            ...
            <p className="text-sm text-[#8892b0] leading-relaxed">
              {error
                ? `Error: ${error}`
                : "The news pipeline is warming up. Articles will appear after the next ingest run (every 30 min)."}
            </p>
            <Button
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 py-5 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight">News & Catalysts</h1>
          <p className="text-xs text-[#8892b0] mt-0.5">
            {articles.length} articles · tagged to prediction markets
          </p>
        </div>
        <Button
          onClick={fetchArticles}
          ...
          <div className="flex flex-wrap gap-1">
            {article.tags.slice(0, 3).map((tag) => (
              <Link
                ...
                className="text-[10px] bg-[#1a1e2e] border border-[#2f374f] text-[#8892b0] hover:text-[#57D7BA] hover:border-[#57D7BA]/30 rounded px-2 py-0.5 transition-colors truncate max-w-[180px]"
                title={tag.question || tag.market_id}
              >
                {(tag.question || tag.market_id).slice(0, 50)}
                {(tag.question || "").length > 50 ? "…" : ""}
              </Link>
            ))}
            {article.tags.length > 3 && (
              <span className="text-[10px] text-[#8892b0]/50 px-1 py-0.5">
