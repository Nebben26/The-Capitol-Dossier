...
Freshness by endpoint:
  ...
  /spread-history    → point-in-time snapshots at each ingest cycle`}</FormulaBlock>
      ...
      {/* ─── Market Briefs Methodology ─── */}
      <div id="briefs" />
      <Section title="Market Briefs">
        <Card>
          <Label>Generation method</Label>
          <FormulaBlock>{`Each brief is generated deterministically from live Supabase data:

1. Top movers    → markets in category ordered by |change_24h|, top 3
2. Top arb       → disagreements in category ordered by spread DESC, top 1
3. Resolutions   → resolved markets ordered by resolves_at DESC, last 3
4. Quiver index  → current_value + change_24h from quiver_indices table

No LLM calls — pure template with live data slotted in.
Runs at the end of each ingest cycle (~every 30 minutes).`}</FormulaBlock>
          <Body>
            Briefs are generated by a deterministic template engine, not a language model. This means
            they are perfectly reproducible given the same data, never hallucinate facts, and run in
            milliseconds. The tradeoff is fixed structure — the narrative is templated, not freeform.
          </Body>
        </Card>
        <Card>
          <Label>Output formats</Label>
          <FormulaBlock>{`Markdown  → clean .md with emoji section headers, linkable
HTML      → email-safe inline styles, 600px max-width, dark theme
JSON      → structured envelope: { category, generated_at, index,
              movers[], top_arb, resolutions[], footer }

Custom branding (Pro+):
  Appended to footer of all three formats.
  Set via /settings/briefs or POST /api/settings/briefs.`}</FormulaBlock>
          <Body>
            All three formats are generated simultaneously and stored in the same database row.
            Requesting a different format does not trigger a regeneration — it reads the pre-computed
            column from the last ingest cycle. The <code className="text-[#f0f6fc] bg-[#0d1117] px-1 rounded">generated_at</code> field
            reflects when the brief was last written, not when you requested it.
          </Body>
        </Card>
        <Card>
          <Label>Distribution channels</Label>
          <FormulaBlock>{`RSS feed:    /api/briefs/[category]/rss — 30 most recent briefs per category
JSON API:   /api/briefs/[category]?format=json|markdown|html — latest brief
Embed:      data-quiver-widget="brief" data-id="[category]" — live widget card
Dashboard:  /briefs — human-readable with format toggle and copy button`}</FormulaBlock>
          <Body>
            All channels are publicly readable without authentication. Custom branding is applied at
            generation time and stored in the brief — the RSS and API outputs for a Pro subscriber
            already include the custom footer. No runtime substitution is needed.
          </Body>
        </Card>
      </Section>
