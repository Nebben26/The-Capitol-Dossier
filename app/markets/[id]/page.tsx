...
// ─── DEPTH CHART ──────────────────────────────────────────────────────
function DepthChart() {
  const bidData = orderbookBids.map((b) => ({ price: b.price, bids: b.total, asks: 0 }));
  const askData = orderbookAsks.map((a) => ({ price: a.price, bids: 0, asks: a.total }));
...
export default function MarketDetailPage() {
  ...
  const market = marketById[id] ?? marketById["recession-2026"];
          ...
          <Card className="bg-[#222638] border-[#2a2f45] lg:min-w-[280px] shrink-0">
            <CardContent className="p-5">
              ...
              <div className="flex gap-2 mt-4">
                <Button className="flex-1 h-9 bg-[#22c55e] text-white hover:bg-[#22c55e]/80 font-semibold text-sm">
                  Buy YES
                </Button>
                <Button className="flex-1 h-9 bg-[#ef4444] text-white hover:bg-[#ef4444]/80 font-semibold text-sm">
                  Buy NO
                </Button>
              </div>
            </CardContent>
